...
: Node("darknet_detection_node")
{
  ...
  detection_pub_ = create_publisher<vision_msgs::msg::Detection2DArray>(
    "output_detection_2d", rclcpp::SensorDataQoS().reliable());  /*Por que se ponia aqui rclcpp::SensorDataQoS().reliable() y no 10*/
}
  ...
  const darknet_ros_msgs::msg::BoundingBoxes::ConstSharedPtr & msg)
{
  if (detection_pub_->get_subscription_count() > 0) {
    /* Array de detecciones, para publicar todas las bbx detectadas en la imagen. */
    vision_msgs::msg::Detection2DArray detection_array_msg;
    detection_array_msg.header = msg->image_header;
    
    /* Se escanean todas las bounding boxes detectadas para publicarlas en un topic
    para que otros nodos de ROS puedan analizarlas si quieren. */
    for (const auto & bbx : msg->bounding_boxes) {
      /* Se crea un objeto Detection2D a partir del cual se calcula el centro de la bbx
